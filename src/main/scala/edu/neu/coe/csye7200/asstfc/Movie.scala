...
object Movie extends App {
  ...
  object MoviesProtocol extends DefaultJsonProtocol {
    // 20 points
    // TO BE IMPLEMENTED
    implicit val ratingFormat: JsonFormat[Rating] = jsonFormat2(Rating.apply)
    implicit val nameFormat: JsonFormat[Name] = jsonFormat4(Name.apply)
    implicit val principalFormat: JsonFormat[Principal] = jsonFormat2(Principal.apply)
    implicit val formatFormat: JsonFormat[Format] = jsonFormat4(Format.apply)
    implicit val productionFormat: JsonFormat[Production] = jsonFormat4(Production.apply)
    implicit val reviewsFormat: JsonFormat[Reviews] = jsonFormat7(Reviews.apply)
    implicit val movieFormat: JsonFormat[Movie] = jsonFormat11(Movie.apply)
    // END SOLUTION
  }
  ...
  def testSerializationAndDeserialization(ms: Seq[Movie]): Boolean = {
    ...
    val json = ms.toJson.prettyPrint
    val deserialized = json.parseJson.convertTo[Seq[Movie]]
    ms == deserialized
  }
